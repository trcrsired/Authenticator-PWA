...
import { isOtpWasmReady, otpGenerate } from "./wasm-otp";
...
export class KeyUtilities {
  ...
  static generate(
    ...
    // GOST algorithms stay on the JS path; everything else is generated by the
    // fast_io wasm module when it's loaded.
    const isGost =
      algorithm === OTPAlgorithm.GOST3411_2012_256 ||
      algorithm === OTPAlgorithm.GOST3411_2012_512;
    if (!isGost && isOtpWasmReady()) {
      let wasmCounter = counter;
      if (type !== OTPType.hotp && type !== OTPType.hhex) {
        let epoch = Math.round(new Date().getTime() / 1000.0);
        if (clockOffset) {
          epoch = epoch + Number(clockOffset);
        }
        wasmCounter = Math.floor(epoch / period);
      }
      return otpGenerate(
        type,
        secret,
