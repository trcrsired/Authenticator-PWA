...
export default Vue.extend({
  ...
  created() {
    // Drag only from the move handle while editing — lets touch users scroll
    // the list normally and still reorder via the grip.
    this.$dragula.$service.options("entryDrake", {
      moves: (
        ...
        return Boolean(handle.closest(".movehandle"));
      },
      invalid: () => {
        if (!this.$store.state.style.style.isEditing) {
          return true;
